...
class LeCartCustom(LeBasecart):
	...
	def get_categories_ext_export(self, categories):
		...
		categories_des = self.select_multiple_data_connector(categories_ext_queries)
		language_ids = duplicate_field_value_from_list(categories_des['data']['categories_description'], 'language_id')
		categories_moreext_queries = {
			'language':{
				'type': 'select',
				'query': "SELECT * FROM  _DBPRF_languages WHERE languages_id IN " + self.list_to_in_condition(language_ids),
			}
		}
		categories_language = self.select_multiple_data_connector(categories_moreext_queries)
		categories_ext = self.sync_connector_object(categories_des, categories_language)
	...
	def convert_category_export(self, category, categories_ext):
		...
		# self.log(category_description,"log")
	...
	def get_products_ext_export(self, products):
		...
		product_ext_queries = self.get_connector_data(url_query, {
			'serialize': True, 'query': json.dumps(product_ext_queries)
		})
		language_ids = duplicate_field_value_from_list(product_ext_queries['data']['products_description'], 'language_id', )
		...
		product_ext_rel_queries = self.get_connector_data(url_query, {
			'serialize': True, 'query': json.dumps(product_ext_rel_queries)
		})
		product_ext = self.sync_connector_object(product_ext_queries, product_ext_rel_queries)		
	...
	def convert_product_export(self, product, products_ext):
		...
		product_description = get_list_from_list_by_field(products_ext_data['products_description'], 'products_id',product['products_id'])
		...
		product_categories = get_list_from_list_by_field(products_ext_data['product_categories'], 'products_id',
		                                                 product['products_id'])
	...
	def convert_customer_export(self, customer, customers_ext):
		...
		address_books = get_list_from_list_by_field(customers_ext['data']['address_book'], 'address_book_id',customer['customers_id'])
		customers_info = get_list_from_list_by_field(customers_ext['data']['customers_info'], 'customers_info_id',customer['customers_id'])
		# if customers_info:
    	# customer_data['created_at'] = customers_info[0]['customers_info_date_account_created']
		if address_books:
			for address_book in address_books:
				address_data = self.construct_customer_address()
				address_data['code'] = 'AL'
				...
				zone = get_row_from_list_by_field(customers_ext['data']['zones'], 'zone_country_id',address_book['entry_country_id'])
				if country:
				...
				# if zone:
				if zone:
					address_data['state']['id'] = zone['zone_id']
					address_data['state']['state_code'] = zone['zone_code']
					address_data['state']['name'] = zone['zone_name']
					address_data['state']['code'] = zone['zone_country_id']
	...
	def get_orders_ext_export(self, orders):
		...
		orders_ext_queries = {
			...
			'countries': {
				'type': 'select',
				'query': "SELECT * FROM countries WHERE countries_name IN " + self.list_to_in_condition(country_ids)
			},
	...
	def convert_order_export(self, order, orders_ext):
		...
		# flname = data.split()
		# order_customer['first_name'] = flname[1]
		# order_customer['last_name'] = flname[0]
		...
		data = order['delivery_name']
		flname = data.split()
		customer_address['id'] = order['orders_id']
		customer_address['code'] = order['client_customers_id']
		customer_address['first_name'] = flname[1]
		customer_address['last_name'] = flname[0]
		...
		customer_country = get_row_from_list_by_field(orders_ext['data']['countries'], 'countries_name', order['delivery_country'])
		if customer_country:
			...
			customer_address['country']['code'] = customer_country['countries_iso_code_3']
		...
		if customer_state:
			...
			customer_address['state']['code'] = customer_state['zone_country_id']
		...
		billing_name = self.get_name_from_string(order['billing_name'])
		data = order['billing_name']
		flname = data.split()
		order_billing['first_name'] = flname[1]
		order_billing['last_name'] = flname[0]
		...
		order_billing['company'] = order['billing_country']
		...
		if billing_state:
			...
			order_billing['state']['code']=billing_state['zone_country_id']
		...
		delivery_name = self.get_name_from_string(order['delivery_name'])
		data = order['delivery_name']
		flname = data.split()
		order_delivery['first_name'] = flname[1]
		order_delivery['last_name'] = flname[0]
		...
		payments = get_row_from_list_by_field(orders_ext['data']['orders_payment'], 'orders_id', order['orders_id'])
		order_payment = self.construct_order_payment()
		if payments:
			order_payment['title'] = payments['title']
			order_payment['id'] = payments['orders_total_id']    			
		...
		for order_product in order_products:
			...
			order_item['created_at'] = order['date_purchased']
		...
		#
		# self.log(order_data, "order data")
